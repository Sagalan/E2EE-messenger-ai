...
def login():
    ...
    return jsonify({"status":"ok","user_id":uid,"priv":user_keys[uid],"pub":users[uid]})
...
def send():
    ...
    if 'message' in data:
        # Открытый текст — шифруем на сервере
        if sender in user_keys and to in users:
            ...
            # Пользователь ещё не залогинен — сохраняем как есть
